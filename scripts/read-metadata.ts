...
// CHANGE: Prettier-aligned formatting to satisfy lint.
// WHY: Lint check flagged this script; formatting ensures invariant "Верификация: через линтер".
...
(async () => {
  ...
  try {
    ...
    const metadataAccount = await program.account.tokenMetadata.fetch(
      metadataPDA
    );
    ...
    console.log(
      "👤 Update Authority:",
      metadataAccount.updateAuthority.toBase58()
    );
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("");
    console.log("💡 Важно:");
    ...
    if (error.code === "AccountNotFound") {
      ...
      console.log(
        "   Запустите скрипт add-metadata.ts для создания метаданных."
      );
    } else {
